...
from tools.model.augmentations import ElasticSize, ElasticSize2
...
LOSS = sm_losses.binary_crossentropy + sm_losses.JaccardLoss(per_image=False)      ##sm_losses.bce_jaccard_loss  ## sm_losses.binary_crossentropy + sm_losses.DiceLoss(smooth=1)
INPUT_SIZE = (352, 352)
BATCH_SIZE = 8
...
AUGMENTATIONS = albumentations.Compose(transforms=[
    ...
    # albumentations.Lambda(image=elastic_size, mask=elastic_size, p=0.5),
    # ElasticSize(p=0.5),
    ElasticSize2(p=0.5),

    # albumentations.IAACropAndPad(percent=(-0.3, 0.3), pad_cval=1, p=1),

#    albumentations.Rotate(limit=20, border_mode=cv2.BORDER_CONSTANT, p=1),
#    albumentations.IAACropAndPad(percent=(-0.3, 0.3), p=1),


    # albumentations.OneOrOther(
    #     first=albumentations.Compose(transforms=[
    #         albumentations.ShiftScaleRotate(
    #             border_mode=cv2.BORDER_CONSTANT, rotate_limit=20, shift_limit=0.2, scale_limit=0.2, p=1),
    #         ElasticSize(p=0.5),
    #     ]),
    #     second=albumentations.Compose(transforms=[
    #         albumentations.IAACropAndPad(percent=(-0.3, 0.3), p=1),
    #         albumentations.Rotate(limit=20, border_mode=cv2.BORDER_CONSTANT, p=1)
    #     ])),
